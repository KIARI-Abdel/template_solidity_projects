...
- `.env` (_Needs to be created manually_): For development an `.env` file containing several environment variables must exist in the root directory of the project. The provided `.env.example` file serves as a template. At the moment the most important entries are the following:
  ...
  - `PRIVATE_KEY`: Private key is essential variable that is using for signing transactions. In order for successful transactions the account with provided private key should be certified and have enough funds.
  - [Optional] - `MNEMONIC`: Mnemonic phrase of the HD wallet used for interacting with the networks (i.e. private key). Contracts are provisioned from an account (derived from the mnemonic) which should have enough funds to provision the contracts.
...
Alternatively you can deploy from CLI by using commands below.

##### How to Deploy on BC testnetwork from CLI


``` npm run deploy:bcbc_test```

##### How to Deploy on BC Live network from CLI

 ``` npm run deploy:bcbc_live```

##### How to Deploy on Goerli network from CLI

  ``` npm run deploy:staging```

##### How to Verify on Goerli network from CLI

  ``` npm run verify:staging```

##### How to Deploy on Ethereum network from CLI

  ``` npm run deploy:ethereum ```

##### How to Verify on Ethereum network from CLI
